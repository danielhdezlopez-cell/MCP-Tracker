...
export type Theme = 'neon-blue' | 'comic-ink' | 'cap-first-avenger' | 'convocation' | 'dark-dimension' | 'dracula' | 'hellfire-club' | 'hulkbuster' | 'hydra' | 'mephisto' | 'midnight-sons' | 'shield' | 'asgard' | 'miles-morales' | 'spider-man' | 'thanos' | 'ultron';
  ...
  persist(
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        if (state && !(['cap-first-avenger', 'convocation', 'dark-dimension', 'dracula', 'hellfire-club', 'hulkbuster', 'hydra', 'mephisto', 'midnight-sons', 'shield', 'asgard', 'miles-morales', 'spider-man', 'thanos', 'ultron'] as string[]).includes(state.theme)) {
          state.theme = 'shield';
        }
