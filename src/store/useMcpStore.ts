...
export type Theme = 'neon-blue' | 'comic-ink' | 'apocalypse' | 'cable' | 'captain-america' | 'cap-first-avenger' | 'convocation' | 'cyclops' | 'dark-dimension' | 'doc-ock' | 'dr-strange' | 'dracula' | 'green-goblin' | 'hellfire-club' | 'hulkbuster' | 'hydra' | 'magik' | 'magneto' | 'mephisto' | 'midnight-sons' | 'onslaught' | 'professor-x' | 'sentinels' | 'shield' | 'asgard' | 'miles-morales' | 'spider-man' | 'thanos' | 'the-leader' | 'thunderbolts' | 'ultron' | 'weapon-x';
  ...
  persist(
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        if (state && !(['apocalypse', 'cable', 'captain-america', 'cap-first-avenger', 'convocation', 'dark-dimension', 'doc-ock', 'dr-strange', 'dracula', 'green-goblin', 'hellfire-club', 'hulkbuster', 'hydra', 'magik', 'magneto', 'mephisto', 'midnight-sons', 'onslaught', 'professor-x', 'sentinels', 'shield', 'asgard', 'miles-morales', 'spider-man', 'thanos', 'thunderbolts', 'ultron', 'weapon-x'] as string[]).includes(state.theme)) {
          state.theme = 'shield';
        }
