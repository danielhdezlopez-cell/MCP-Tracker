...
export type Theme = 'neon-blue' | 'comic-ink' | 'apocalypse' | 'cap-first-avenger' | 'convocation' | 'cyclops' | 'dark-dimension' | 'dracula' | 'hellfire-club' | 'hulkbuster' | 'hydra' | 'magik' | 'mephisto' | 'midnight-sons' | 'onslaught' | 'professor-x' | 'sentinels' | 'shield' | 'asgard' | 'miles-morales' | 'spider-man' | 'thanos' | 'the-leader' | 'thunderbolts' | 'ultron' | 'weapon-x';
  ...
  persist(
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        if (state && !(['apocalypse', 'cap-first-avenger', 'convocation', 'dark-dimension', 'dracula', 'hellfire-club', 'hulkbuster', 'hydra', 'magik', 'mephisto', 'midnight-sons', 'onslaught', 'professor-x', 'sentinels', 'shield', 'asgard', 'miles-morales', 'spider-man', 'thanos', 'thunderbolts', 'ultron', 'weapon-x'] as string[]).includes(state.theme)) {
          state.theme = 'shield';
        }
