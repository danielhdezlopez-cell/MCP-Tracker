...
const SCAN_DUR = 780; // keep in sync with --scan-dur in CSS
...
export const VSLeaderPanel = memo(function VSLeaderPanel({ side, showPortrait }: Props) {
  ...
  useEffect(() => {
    ...
    // Only animate leader→leader swaps; skip first assignment (null→leader)
  ...
  return (
    <div
      ...
      aria-label={ariaLabel}
    >
      {/* Two persistent layers; only one visible at a time outside of transitions */}
      ...
      {/* Scan bar — sits above layers, below frame */}
